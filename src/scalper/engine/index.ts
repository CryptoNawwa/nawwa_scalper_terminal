...
import { handleError, ScalperError } from '../../_common/error';
...
export class Engine {
  ...
  private async doAutoTakeProfitSytem(positions: Position[], exchange: ExchangeMetadata, atpData: ATPData) {
    ...
    await bluebird.each(positions, async position => {
      ...
      if (!symbol) {
        throw new ScalperError(`ATP - Cannot find symbol ${position.symbol}`);
      }
  ...
  private async callbackPositionUpdated(from: SUPPORTED_EXCHANGE, updatedPositions: Position[]) {
    ...
    if (!activeExchange) {
      ...
      throw new ScalperError(EngineErrorReason.NO_ACTIVE_EXCHANGE_FOUND);
    }
  ...
  private async connectToNewExchange(connectTo: SUPPORTED_EXCHANGE) {
    ...
    switch (connectTo) {
      ...
      case SUPPORTED_EXCHANGE.bybit:
        ...
        break;
      default:
        throw new ScalperError(EngineErrorReason.EXCHANGE_NOT_SUPORTED);
    }
  ...
  private getCommandOrShortcut(userInput: string) {
    const shortcut = Object.entries(this.shortcuts.getData()).find(([key]) => {
      if (key.toLowerCase() === userInput.toLowerCase()) return true;
    });

    if (!shortcut) {
      return userInput.toUpperCase().split(' ');
    }
  ...
  public async runCommand(userInput: string) {
    try {
      if (userInput == '') return;
      const [command, ...commandArgs] = this.getCommandOrShortcut(userInput);
